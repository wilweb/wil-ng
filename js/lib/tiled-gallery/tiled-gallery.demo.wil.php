<html>
	<head>
		<script type='text/javascript' src='http://wyan.htpwebdesign.ca/wordpress/wp-includes/js/jquery/jquery.js?ver=1.8.3'></script>
	</head>
...
$tiledGallery = new Jetpack_Tiled_Gallery();
...
class Jetpack_Tiled_Gallery {
	...
	public function get_attachments_wil() {
		...
		// $attachments is an array of paths/filename to images

		$root = $_SERVER['DOCUMENT_ROOT'];
		$host = "http://".$_SERVER['HTTP_HOST'];
		$images_path = "/portfolio/sites/all/themes/wil_commerce_kickstart_theme/photos/";

		$images_root_path = $root.$images_path;
		$images_link = $host.$images_path;

		//echo "<h1>images_src = " .$images_link. "</h1>";
	...
	//public function gallery_shortcode( $val, $atts ) {
		...
		//echo "<h1>get_attachments_wil</h1>";
		...
		foreach ( $grouper->grouped_images as $row ) {
			...
			foreach( $row->groups as $group ) {
				...
				foreach ( $group->images as $image ) {
					...
					$output .= '<div class="tiled-gallery-item ' . $size  . '"><a href="' .  $link  . '"><img ' . ' src="' .  $img_src  . '" width="' .  $image->width  . '" height="' .  $image->height  . '" align="left"' . ' /></a>';
...
//)
	...
	public function get_images_with_sizes_wil( $attachments ) {
		...
		foreach ( $attachments as $image ) {
			...
			$image->post_title = "test_post_title";
...
?>
	...
	<link rel='stylesheet' id='tiled-gallery-css'  href='http://wyan.htpwebdesign.ca/wordpress/wp-content/plugins/slimjetpack/modules/tiled-gallery/tiled-gallery/tiled-gallery.css?ver=2012-09-21' type='text/css' media='all' />

	<script type='text/javascript' src='http://wyan.htpwebdesign.ca/wordpress/wp-content/plugins/slimjetpack/modules/tiled-gallery/tiled-gallery/tiled-gallery.js?ver=3.5.1'></script>
